...
/* ─────────────────────────────────────────────
   WHATSAPP RELATÓRIO SEMANAL INDIVIDUAL
───────────────────────────────────────────── */
function buildWhatsAppRelatorioSemanal(emp, records, reportDates, school) {
  const apoio = IS_APOIO(emp.role);
  ...
  let lines = [];
  lines.push(`📊 *RELATÓRIO SEMANAL DE FREQUÊNCIA*`);
  lines.push(`🏫 *${school.name || "FreqSchool"}*`);
  if (school.city) lines.push(`📍 ${school.city}`);
  lines.push(``);
  lines.push(`👤 *${emp.name}*`);
  lines.push(`💼 ${emp.role}`);
  lines.push(`📅 Semana: ${dataInicio} a ${dataFim}`);
  lines.push(`─────────────────`);

  let totalPresente = 0, totalAusente = 0, totalJust = 0, totalFolga = 0;

  reportDates.forEach(d => {
    const dow = DS[new Date(d + "T12:00:00").getDay()];
    const label = `${dow} ${fmtD(d)}`;
    if (apoio) {
      let linha = `${label}:`;
      TURNOS.forEach(turno => {
        const s = records[recordKey(d, emp.id, turno)] || null;
        const tLabel = turno === "manha" ? "M" : "T";
        linha += ` [${tLabel}: ${s ? abbr[s].split(" ")[0] : "➖"}]`;
        if (s === "presente") totalPresente++;
        else if (s === "ausente") totalAusente++;
        else if (s === "justificado") totalJust++;
        else if (s === "folga") totalFolga++;
      });
      lines.push(linha);
    } else {
      const s = records[recordKey(d, emp.id)] || null;
      lines.push(`${label}: ${s ? abbr[s] : "➖ Não registrado"}`);
      if (s === "presente") totalPresente++;
      else if (s === "ausente") totalAusente++;
      else if (s === "justificado") totalJust++;
      else if (s === "folga") totalFolga++;
  ...
  if (totalAusente > 0) {
    lines.push(``);
    lines.push(`⚠️ Você possui ${totalAusente} falta(s) nesta semana.`);
    lines.push(`Para justificar, acesse:`);
...
export default function App() {
  ...
  function sendNextReportWa() {
    const emp = reportWaQueue[reportWaIndex];
    const message = buildWhatsAppRelatorioSemanal(emp, records, reportDates, school);
    sendWhatsApp(emp.phone, message);
    if (reportWaIndex < reportWaQueue.length - 1) {
      setReportWaIndex(i => i + 1);
    } else {
      setShowReportWa(false);
      showToast(`Relatório enviado para ${reportWaQueue.length} funcionário(s)!`);
    }
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0f172a 0%,#1e293b 50%,#0f172a 100%)", fontFamily: "Georgia,serif", color: "#f1f5f9" }}>
      ...
      {showReportWa && reportWaQueue.length > 0 && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div onClick={() => setShowReportWa(false)} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(5px)" }} />
          <div style={{ position: "relative", width: "100%", maxWidth: 440, background: "#1a2640", border: "1px solid rgba(37,211,102,0.3)", borderRadius: 20, padding: "26px 24px", boxShadow: "0 25px 60px rgba(0,0,0,0.6)" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: "bold", color: "#f1f5f9" }}>📊 Relatório Semanal por WhatsApp</div>
                <div style={{ fontFamily: "sans-serif", fontSize: 12, color: "#64748b", marginTop: 2 }}>Enviando individualmente para cada funcionário</div>
              </div>
            ...
            {(() => {
              const emp = reportWaQueue[reportWaIndex];
              return (
                <div style={{ padding: "14px 16px", background: "rgba(37,211,102,0.07)", border: "1px solid rgba(37,211,102,0.2)", borderRadius: 12, marginBottom: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: avatarColor(emp.id), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: "bold", flexShrink: 0 }}>{getInitials(emp.name)}</div>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600 }}>{emp.name}</div>
                      <div style={{ fontFamily: "sans-serif", fontSize: 12, color: "#94a3b8" }}>{emp.role}</div>
                      <div style={{ fontFamily: "sans-serif", fontSize: 12, color: "#25d366", marginTop: 2 }}>📞 {emp.phone}</div>
                    </div>
                  </div>
                </div>
              );
